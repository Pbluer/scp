    ...
    <div class="flex justify-center items-center">
        <form @submit.prevent="gravar()" class="bg-blue-600px-5 flex flex-col items-center p-5 rounded-md border-2 shadow mt-36 bg-white">         
            ...
            <div class="gap-2">
                ...
                <section class="flex flex-col mt-5 text-left w-[20rem]">
                    <label class="mb-1 font-bold text-[1.2rem] text-gray-600" > {{ form.categoria == 1 ? "CNPJ" : "CPF" }} </label>
                    <input v-show="form.categoria == 2" ref="identificacao1" class="border-2 border-gray-50 bg-gray-50 rounded focus:border-[#0C4A6E] px-2 h-[35px] outline-none" type="text" v-model.trim="form.identificacao" v-mask="'000.000.000-00'" >
                    <input v-show="form.categoria == 1 " ref="identificacao2" class="border-2 border-gray-50 bg-gray-50 rounded focus:border-[#0C4A6E] px-2 h-[35px] outline-none" type="text" v-model.trim="form.identificacao" v-mask="'00.000.00/0000-00'" >
                </section>
...
export default {
    ...
    methods:{
        ...
        async validarFormulario(){
            let identificacao = this.form.identificacao 
            ...
            if( !identificacao ){
                ...
                this.alert.mensage = this.form.categoria == 1 ? 'Insira o CNPJ.' : 'Insira o CPF.'
                this.form.categoria == 1 ? this.$refs.identificacao1.focus() : this.$refs.identificacao2.focus()
                return true
            }            
